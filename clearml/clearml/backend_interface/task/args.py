...
from typing import Tuple, Type, Union, List, Optional, Dict, Any
...
class _Arguments:
    ...
    class _ProxyDictWrite(dict):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._section_name = (
                (__section_name.strip(_Arguments._prefix_sep) + _Arguments._prefix_sep) if __section_name else None
            )
    ...
    def exclude_parser_args(self, excluded_args: dict) -> None:
        """
    ...
    def set_defaults(self, *dicts: Any, **kwargs: Any) -> None:
    ...
    def connect(self, parser: argparse.ArgumentParser) -> None:
    ...
    def _add_to_defaults(
        ...
        descriptions: dict,
        arg_types: dict,
        a_args: Union[list, None] = None,
        a_namespace: Union[argparse.Namespace, None] = None,
        a_parsed_args: Union[dict, argparse.Namespace, None] = None,
    ) -> Tuple[dict, dict, dict]:
        # noinspection PyProtectedMember
        actions = [a for a in a_parser._actions if isinstance(a, _StoreAction) or isinstance(a, _StoreConstAction)]
        args_dict = {}
    ...
    def copy_defaults_from_argparse(
        ...
        parsed_args: Union[argparse.Namespace, dict, None] = None,
    ) -> None:
        (
            task_defaults,
    ...
    def _get_defaults_from_argparse(
        self,
        parser: argparse.ArgumentParser,
        args: Union[List[str], None] = None,
        namespace: Union[argparse.Namespace, None] = None,
        parsed_args: Union[argparse.Namespace, Dict[str, Any], None] = None,
    ) -> Tuple[Dict[str, Any], Dict[str, str], Dict[str, Union[Type, None]]]:
        task_defaults = {}
        task_defaults_descriptions = {}
        task_defaults_types = {}
        self._add_to_defaults(
            parser,
            task_defaults,
            task_defaults_descriptions,
            task_defaults_types,
        ...
        if Session.check_min_api_version("2.9"):
            ...
            task_defaults_descriptions = dict(
                [
                    (self._prefix_args + k, v)
                    for k, v in task_defaults_descriptions.items()
                    if self._exclude_parser_args.get(k, include_all)
                ]
            )
            task_defaults_types = dict(
                [
                    (self._prefix_args + k, v)
                    for k, v in task_defaults_types.items()
                    if self._exclude_parser_args.get(k, include_all)
                ]
            )
        else:
    ...
    @classmethod
    def _find_parser_action(
        cls, a_parser: argparse.ArgumentParser, name: str
    ) -> List[Tuple[argparse.ArgumentParser, argparse.Action]]:
        # find by name
        # noinspection PyProtectedMember
    ...
    @classmethod
    def _remove_req_flag_from_mutex_groups(cls, parser: argparse.ArgumentParser) -> None:
        # noinspection PyBroadException
        try:
            # noinspection PyProtectedMember
    ...
    def copy_to_parser(
        self,
        parser: argparse.ArgumentParser,
        parsed_args: Union[argparse.Namespace, dict, None],
    ) -> None:
        def cast_to_bool_int(value: Union[str, int], strip: bool = False) -> Optional[Union[bool, int]]:
        ...
        task_arguments = dict(
            [
                ...
                if k.startswith(prefix) and self._exclude_parser_args.get(k[len(prefix) :], True)
            ]
        )
            ...
            for parent_parser, current_action in matched_actions:
                if current_action and current_action.default == SUPPRESS and not v:
                    ...
                    const_value = (
                        ...
                        else (current_action.default if current_action.default is not None else True)
                    )
                    ...
                    or isinstance(current_action, _AppendAction)
                ):
                    # noinspection PyBroadException
                    try:
                    ...
                    if var_type == int:
                        ...
                        self.__cast_arg(current_action.default)
                    ) == str(v):
                    ...
                    try:
                        if current_action.default is None and current_action.type != str and not v:
                            ...
                            not isfunction(current_action.type) and current_action.default == current_action.type(v)
                        ) or (
                            isfunction(current_action.type) and str(self.__cast_arg(current_action.default)) == str(v)
                        ):
    ...
    def copy_from_dict(
        ...
        descriptions: Union[dict, None] = None,
        param_types: Union[dict, None] = None,
    ) -> Union["_Arguments._ProxyDictWrite", dict]:
        # add dict prefix
    ...
    def copy_to_dict(self, dictionary: dict, prefix: str = None) -> Union["_Arguments._ProxyDictReadOnly", dict]:
        ...
        if prefix:
            prefix = prefix.strip(self._prefix_sep) + self._prefix_sep
            parameters = dict(
                [(k[len(prefix) :], v) for k, v in self._task.get_parameters().items() if k.startswith(prefix)]
            )
            # noinspection PyProtectedMember
            parameters_type = {
                k: p.type
                for k, p in (
                    (self._task._get_task_property("hyperparams", raise_on_error=False) or {}).get(
                        prefix[: -len(self._prefix_sep)]
                    )
                    or {}
                ).items()
                if p.type
            ...
            parameters_type = {}
            parameters = dict(
                [(k, v) for k, v in self._task.get_parameters().items() if not k.startswith(self._prefix_tf_defines)]
            )
    ...
    @classmethod
    def get_supported_types(cls, as_str: bool = False) -> Union[Type, Tuple[str]]:
        """
    ...
    @classmethod
    def __cast_arg(
        cls, arg: Any, dtype: Optional[Union[Type[float], Type[int]]] = None
    ) -> Union[str, Any, List[Optional[Union[float, int]]]]:
    ...
    @property
    def prefix_tf_defines(self) -> None:
        return self._prefix_tf_defines

    @property
    def prefix_sep(self) -> None:
