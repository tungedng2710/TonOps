...
class TaskModels(UserDict):
    ...
    @property
    def input(self) -> ModelsList:
        return self._input

    @property
    def output(self) -> ModelsList:
        return self._output
    ...
    def _get_input_models(self, task: "Task") -> ModelsList:
        # noqa: F821
        ...
        if Session.check_min_api_version("2.13") and task._get_task_property(
            ...
            input_models = OrderedDict((x.name, get_model(x.model)) for x in task.data.models.input)
        else:
    ...
    @staticmethod
    def _get_output_models(task: "Task") -> ModelsList:
        # noqa: F821

        res = task.send(models.GetAllRequest(task=[task.id], order_by=["created"], only_fields=["id"]))
        ...
        output_models = OrderedDict((resolve_name(i, m_id), Model(model_id=m_id)) for i, m_id in enumerate(ids))
