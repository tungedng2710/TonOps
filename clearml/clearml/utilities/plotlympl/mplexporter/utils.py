...
import itertools
import warnings
from typing import Dict, Tuple, Union, List, Iterator, Any
...
def export_color(color: Any) -> str:
...
def _many_to_one(input_dict: Dict[Tuple[Union[str, int]], Any]) -> Dict[Union[str, int], Any]:
...
def get_dasharray(obj: matplotlib.artist.Artist) -> str:
...
def get_path_style(path: matplotlib.path.Path, fill: bool = True) -> dict:
...
def get_line_style(line: matplotlib.lines.Line2D) -> dict:
...
def get_marker_style(line: matplotlib.lines.Line2D) -> dict:
...
def get_text_style(text: Any) -> dict:
...
def get_axis_properties(axis: Any) -> dict:
...
def get_grid_style(axis: Any) -> dict:
...
def get_figure_properties(fig: Any) -> dict:
    return {
...
def get_axes_properties(ax: Any) -> dict:
    props = {
...
def iter_all_children(obj: Any, skipContainers: bool = False) -> Iterator[Any]:
    """
...
def get_legend_properties(ax: Any, legend: Any) -> dict:
