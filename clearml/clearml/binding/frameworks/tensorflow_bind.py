...
class TensorflowBinding:
    ...
    def update_current_task(
        cls,
        task: Any,
        patch_reporting: bool = True,
        patch_model_io: bool = True,
        report_hparams: bool = True,
    ) -> None:
        if not task:
...
class IsTensorboardInit:
    ...
    @classmethod
    def tensorboard_used(cls) -> bool:
        return cls._tensorboard_initialized

    @classmethod
    def set_tensorboard_used(cls) -> None:
        cls._tensorboard_initialized = True

    @classmethod
    def clear_tensorboard_used(cls) -> None:
        cls._tensorboard_initialized = False

    @staticmethod
    def _patched_tb__init__(original_init: Any, self: Any, *args: Any, **kwargs: Any) -> Any:
...
class ProxyEventsWriter:
    ...
    def _get_sentinel_event(self) -> Any:
        ret = None
    ...
    def get_logdir(self) -> Union[str, None]:
        ret = None
        for ev in self._events:
...
class PatchModelCheckPointCallback:
    ...
    @staticmethod
    def trains_object(self) -> Union[OutputModel, Dict[str, Any]]:
    ...
    def _patched_getattribute(self, attr: str) -> Any:
        ...
        output_model = OutputModel(
            ...
            or PatchModelCheckPointCallback._current_task.get_labels_enumeration(),
...
class PatchTensorFlowEager:
    ...
    @staticmethod
    def _create_summary_file_writer(original_fn: Callable, *args: Any, **kwargs: Any) -> Any:
    ...
    @staticmethod
    def _get_event_writer(writer: Any) -> EventTrainsWriter:
        if not PatchTensorFlowEager._current_task:
            return None
        if not PatchTensorFlowEager.__trains_event_writer.get(id(writer)):
            ...
            try:
                ...
                try:
                    ...
                    # noinspection PyBroadException
                    try:
                        from tensorflow.python.ops.summary_ops_v2 import (
                            _summary_state,
                        )  # noqa
            ...
            PatchTensorFlowEager.__trains_event_writer[id(writer)] = EventTrainsWriter(
                logger=PatchTensorFlowEager._current_task.get_logger(),
                logdir=logdir,
    ...
    @staticmethod
    def trains_object(self) -> Union[EventTrainsWriter, None]:
        ...
        if event_writer and isinstance(step, int) or hasattr(step, "numpy"):
            ...
            try:
                ...
                if plugin_type.startswith("scalars"):
                    event_writer._add_scalar(tag=str(tag), step=tweak_step(step), scalar_data=tensor.numpy())
                elif plugin_type.startswith("images"):
                    img_data_np = tensor.numpy()
                    PatchTensorFlowEager._add_image_event_helper(
                        event_writer, img_data_np=img_data_np, tag=tag, step=step, **kwargs
                    )
                elif plugin_type.startswith("histograms"):
                    event_writer._add_histogram(tag=str(tag), step=tweak_step(step), hist_data=tensor.numpy())
                elif plugin_type.startswith("text"):
                    event_writer._add_text(tag=str(tag), step=tweak_step(step), tensor_bytes=tensor.numpy())
    ...
    @staticmethod
    def _write_scalar_summary(
        writer: Any, step: Union[int, Any], tag: Any, value: Any, name: Optional[str] = None, **kwargs: Any
    ) -> Any:
        if not PatchTensorFlowEager._current_task:
        ...
        if event_writer and isinstance(step, int) or hasattr(step, "numpy"):
            try:
                event_writer._add_scalar(tag=str(tag), step=tweak_step(step), scalar_data=value.numpy())
            except Exception as ex:
                LoggerRoot.get_base_logger(TensorflowBinding).warning(str(ex))
        elif event_writer:

            def _report_summary_op(
                a_writer: Any, a_step: Union[int, Any], a_tag: Any, a_value: Any, a_name: Optional[str] = None, **_: Any
            ) -> None:
            ...
            try:
                ...
                if not _context.executing_eagerly():
                    ...
                    py_function(
                        _report_summary_op,
                        inp=[writer, step, tag, value, name],
                        Tout=[],
                    )
            except Exception as ex:
                LoggerRoot.get_base_logger(TensorflowBinding).warning(str(ex))
    ...
    @staticmethod
    def _write_hist_summary(writer: Any, step: Union[int, Any], tag: Any, values: Any, name: Any, **kwargs: Any) -> Any:
        if not PatchTensorFlowEager._current_task:
            return PatchTensorFlowEager.__original_fn_hist(writer, step, tag, values, name, **kwargs)
        event_writer = PatchTensorFlowEager._get_event_writer(writer)
        if event_writer and isinstance(step, int) or hasattr(step, "numpy"):
            try:
                event_writer._add_histogram(tag=str(tag), step=tweak_step(step), hist_data=values.numpy())
            except Exception as ex:
                LoggerRoot.get_base_logger(TensorflowBinding).warning(str(ex))
        elif event_writer:
            ...
            try:
                ...
                if not _context.executing_eagerly():
                    ...
                    py_function(
                        _report_summary_op,
                        inp=[writer, step, tag, values, name],
                        Tout=[],
                    )
            except Exception as ex:
                LoggerRoot.get_base_logger(TensorflowBinding).warning(str(ex))
    ...
    def _write_image_summary(
        ...
        **kwargs: Any
        ...
        if event_writer and isinstance(step, int) or hasattr(step, "numpy"):
            try:
                PatchTensorFlowEager._add_image_event_helper(
                    event_writer, img_data_np=tensor.numpy(), tag=tag, step=step, **kwargs
                )
            except Exception as ex:
                LoggerRoot.get_base_logger(TensorflowBinding).warning(str(ex))
        elif event_writer:
                ...
                **_: Any
                ...
                if isinstance(a_step, int) or hasattr(a_step, "numpy"):
                    try:
                        ...
                        str_tag = str_tag.decode() if isinstance(str_tag, bytes) else str(str_tag)
                        PatchTensorFlowEager._add_image_event_helper(
                            event_writer, img_data_np=a_tensor.numpy(), tag=str_tag, step=a_step, **kwargs
                        )
            ...
            try:
                ...
                if not _context.executing_eagerly():
                    ...
                    py_function(
                        _report_summary_op,
                        inp=[writer, step, tag, tensor, bad_color, max_images, name],
                        Tout=[],
                    )
            except Exception as ex:
                LoggerRoot.get_base_logger(TensorflowBinding).warning(str(ex))
    ...
    @staticmethod
    def _add_image_event_helper(
        event_writer: EventTrainsWriter, img_data_np: np.ndarray, tag: str, step: Union[int, np.ndarray], **kwargs: Any
    ) -> None:
        if img_data_np.ndim == 1 and img_data_np.size >= 3 and (len(img_data_np[0]) < 10 and len(img_data_np[1]) < 10):
    ...
    @staticmethod
    def _nothing_op(*_: Any, **__: Any) -> Any:
...
class PatchKerasModelIO:
    ...
    def _patch_io_calls(
        ...
        keras_saving: Any,
        Functional: Any,
        keras_saving_legacy: Any = None,
        keras_hdf5: Any = None,
        keras_saving_v3: Any = None,
    ) -> None:
        try:
            ...
            if Network is not None:
                ...
                if hasattr(Sequential.from_config, "__func__"):
                    ...
                    Network.from_config = classmethod(
                        _patched_call(Network.from_config.__func__, PatchKerasModelIO._from_config)
                    )
                else:
                ...
                if hasattr(Sequential.from_config, "__func__"):
                    # noinspection PyUnresolvedReferences
                    Functional.from_config = classmethod(
                        _patched_call(
                            Functional.from_config.__func__,
                            PatchKerasModelIO._from_config,
                        )
                    )
                else:
            ...
            if keras_hdf5 is not None:
                keras_hdf5.save_weights_to_hdf5_group = _patched_call(
                    keras_hdf5.save_weights_to_hdf5_group,
                    PatchKerasModelIO._save_weights,
                )
                keras_hdf5.load_weights_from_hdf5_group = _patched_call(
                    keras_hdf5.load_weights_from_hdf5_group,
                    PatchKerasModelIO._load_weights,
                )
                keras_hdf5.load_weights_from_hdf5_group_by_name = _patched_call(
                    keras_hdf5.load_weights_from_hdf5_group_by_name,
                    PatchKerasModelIO._load_weights,
                )
    ...
    @staticmethod
    def _updated_config(original_fn: Callable, self: Any) -> Any:
    ...
    @staticmethod
    def _from_config(original_fn: Callable, *args: Any, **kwargs: Any) -> Any:
        try:
    ...
    @staticmethod
    def _load_weights(original_fn: Callable, self: Any, *args: Any, **kwargs: Any) -> Any:
        # check if we have main task
        if PatchKerasModelIO._current_task is None:
    ...
    @staticmethod
    def _save(original_fn: Callable, self: Any, *args: Any, **kwargs: Any) -> None:
        ...
        if (
            ...
            or not self.trains_out_model
            or not hasattr(self.trains_out_model[-1], "_processed")
            or not self.trains_out_model[-1]._processed
        ):
            PatchKerasModelIO._update_outputmodel(self, *args, **kwargs)

    @staticmethod
    def _save_weights(original_fn: Callable, self: Any, *args: Any, **kwargs: Any) -> None:
        original_fn(self, *args, **kwargs)
        if PatchKerasModelIO._current_task:
            PatchKerasModelIO._update_outputmodel(self, *args, **kwargs)

    @staticmethod
    def _update_outputmodel(self, *args: Any, **kwargs: Any) -> None:
        # check if we have main task
        if not PatchKerasModelIO._current_task:
            return
    ...
    @staticmethod
    def _save_model(original_fn: Callable, model: Any, filepath: str, *args: Any, **kwargs: Any) -> None:
        original_fn(model, filepath, *args, **kwargs)
        if PatchKerasModelIO._current_task:
            PatchKerasModelIO._update_outputmodel(model, filepath)

    @staticmethod
    def _load_model(original_fn: Callable, filepath: str, *args: Any, **kwargs: Any) -> Any:
        if not PatchKerasModelIO._current_task:
        ...
        if False and running_remotely():
            # register/load model weights
            filepath = WeightsFileHandler.restore_weights_file(
                empty, filepath, Framework.keras, PatchKerasModelIO._current_task
            )
...
class PatchTensorflowModelIO:
    ...
    @staticmethod
    def _save(original_fn: Callable, self: Any, sess: Any, save_path: str, *args: Any, **kwargs: Any) -> str:
        ...
        return WeightsFileHandler.create_output_model(
            self, saved_path, Framework.tensorflow, PatchTensorflowModelIO._current_task
        )

    @staticmethod
    def _save_model(original_fn: Callable, obj: Any, export_dir: str, *args: Any, **kwargs: Any) -> None:
        original_fn(obj, export_dir, *args, **kwargs)
        if not PatchKerasModelIO._current_task:
            return
        # store output Model
        WeightsFileHandler.create_output_model(
            obj, export_dir, Framework.tensorflow, PatchTensorflowModelIO._current_task
        )

    @staticmethod
    def _restore(original_fn: Callable, self: Any, sess: Any, save_path: str, *args: Any, **kwargs: Any) -> Any:
        ...
        WeightsFileHandler.restore_weights_file(
            self, save_path, Framework.tensorflow, PatchTensorflowModelIO._current_task
        )
        return model
    ...
    @staticmethod
    def _load(original_fn: Callable, export_dir: str, *args: Any, **saver_kwargs: Any) -> Any:
        if PatchTensorflowModelIO._current_task is None:
    ...
    @staticmethod
    def _ckpt_save(original_fn: Callable, self: Any, file_prefix: str, *args: Any, **kwargs: Any) -> str:
    ...
    @staticmethod
    def _ckpt_write(original_fn: Callable, self: Any, file_prefix: str, *args: Any, **kwargs: Any) -> str:
        checkpoint_path = original_fn(self, file_prefix, *args, **kwargs)
        if PatchTensorflowModelIO._current_task is None:
            return checkpoint_path
        WeightsFileHandler.create_output_model(
    ...
    @staticmethod
    def _ckpt_restore(original_fn: Callable, self: Any, save_path: str, *args: Any, **kwargs: Any) -> Any:
        if PatchTensorflowModelIO._current_task is None:
