...
class TaskMetric(NonStrictDataModel):
    ...
    :param task: Task ID
    :type task: str
    :param metric: Metric name. If not specified then all metrics for this task
        will be returned
    :type metric: str
    """
    ...
    def __init__(self, task: str, metric: Optional[str] = None, **kwargs: Any) -> None:
...
class GetDebugImageSampleRequest(Request):
    ...
    _schema = {
        ...
        "properties": {
            "iteration": {
                "description": "The iteration to bring debugimage from. If not specifiedthen the latest reported imageis retrieved",
                "type": "integer",
            },
            "metric": {"description": "Metric name", "type": "string"},
            "refresh": {
                "description": "If set then scroll state will berefreshed to reflect the latestchanges in the debug images",
                "type": "boolean",
            },
            "scroll_id": {
                "description": "Scroll ID from the previous callto get_debug_image_sample orempty",
                "type": "string",
            },
            "task": {"description": "Task ID", "type": "string"},
...
class NextDebugImageSampleRequest(Request):
    ...
    _schema = {
        ...
        "properties": {
            "navigate_earlier": {
                "description": "If set then get theeither previous variantevent from the currentiteration or (if does notexist) the last variantevent from the previousiteration.\n                       Otherwise next variantevent from the currentiteration or firstvariant event from thenext iteration",
                "type": "boolean",
            },
            "scroll_id": {
                "description": "Scroll ID from the previous callto get_debug_image_sample",
                "type": "string",
            },
            "task": {"description": "Task ID", "type": "string"},
        },
