...
def mp_handler(use_subprocess, data, additional_parameters):
    process = (
        ...
        if use_subprocess
        else multiprocessing.pool.ThreadPool(4)
    )
...
if __name__ == "__main__":
    ...
    if not args.additional_parameters:
        ...
        args.additional_parameters = [
            f"stuff_{str(randint(0, 100))}:some stuff {str(randint(0, 100))}"
    ...
    param = {
        f"args_{args.num_workers}": f"some value {args.num_workers}"
    }
    task.connect(param)
