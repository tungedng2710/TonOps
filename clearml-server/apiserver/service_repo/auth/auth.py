...
from .local_user import LocalDatabaseProvider
...
def authorize_token(jwt_token, service, action, call):
    ...
    try:
        ...
        if LocalDatabaseProvider.enabled():
            user = User.objects(id=token.identity.user).only(
                "username", "status", "auth_version", "must_change_password"
            ).first()
            if token.auth_version is not None:
                if not user or not user.username:
                    raise errors.unauthorized.InvalidToken("invalid token")
                if user.status != "active" or token.auth_version != (user.auth_version or 1):
                    raise errors.unauthorized.InvalidToken("invalid token")
                ...
                if user.must_change_password and f"{service}.{action}" not in password_change_endpoints:
                    raise errors.forbidden.NoWritePermission("password change required")
        return token
...
def authorize_credentials(auth_data, service, action, call):
    ...
    if LocalDatabaseProvider.enabled() and service == "auth" and action == "login":
        candidate = User.objects(username=access_key.strip().lower()).only("id").first()
        if candidate:
            local_user = LocalDatabaseProvider.authenticate(access_key, secret_key, call)
            query = Q(id=local_user.id)
    elif FixedUser.enabled():
    ...
    with translate_errors_context("authorizing request"):
        ...
        if not user:
            if LocalDatabaseProvider.enabled() and service == "auth" and action == "login":
                # This performs the dummy password comparison and records a generic failure.
                LocalDatabaseProvider.authenticate(access_key, secret_key, call)
            raise errors.unauthorized.InvalidCredentials(
                "failed to locate provided credentials"
            )

        if not fixed_user and not local_user:
